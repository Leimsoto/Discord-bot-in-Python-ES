...
CONFIG_FILE = DATA_DIR / "support_config.json"
...
def get_support_role(guild_id: int) -> int:
    """Obtiene el rol de soporte configurado"""
    config = load_config()
    return config.get(str(guild_id), {}).get('support_role_id')

def set_support_role(guild_id: int, role_id: int) -> bool:
    """Configura el rol de soporte"""
    config = load_config()
    if str(guild_id) not in config:
        config[str(guild_id)] = {}
    config[str(guild_id)]['support_role_id'] = role_id
    return save_config(config)

# ============= ASISTENTE DE RECOPILACIÓN =============
...
class InfoCollector:
    ...
    async def process(self, msg: str) -> str:
        ...
        self.data['messages'].append(('user', msg))
        
        # Primera vez = problema
        if not self.data['problem']:
            self.data['problem'] = msg
            needs_specs = any(k in msg.lower() for k in ['lento', 'lag', 'error', 'crash', 'instalar', 'problema', 'no funciona', 'ayuda'])
            
            if needs_specs:
                return "¿Estás usando una **Laptop** o **PC de escritorio**? 💻"
            else:
                return "COMPLETE"
        
        # Extraer info automática
        self._extract_info(msg)
        
        # Límite o info completa
        if self.questions >= 8 or self._is_complete():
            return "COMPLETE"
        
        # Generar siguiente pregunta
        if self.use_ai:
            try:
                response = await self._generate_ai_question()
                if response and len(response) <= 200 and "COMPLETE" not in response.upper():
                    self.questions += 1
                    self.data['messages'].append(('assistant', response))
                    return response
            except:
                pass
        
        # Fallback a preguntas manuales
        response = self._fallback_question()
        self.questions += 1
        self.data['messages'].append(('assistant', response))
        return response
    
    async def _generate_ai_question(self) -> str:
        """Genera pregunta con IA"""
        context = '\n'.join([f"{r}: {m}" for r, m in self.data['messages'][-6:]])
        state = (
            f"Problema: {self.data['problem']}\n"
            f"Dispositivo: {self.data['device']}\n"
            f"OS: {self.data['os']}\n"
            f"RAM: {self.data['ram']}\n"
            f"CPU: {self.data['cpu']}"
        )
        
        prompt = f"""Eres asistente técnico. Haz UNA pregunta corta (max 100 caracteres) para recopilar info.
...
OBJETIVO: Recopilar dispositivo, OS, RAM, CPU según el problema.

Si ya tienes suficiente info, responde SOLO: "COMPLETE"
Sino, haz la siguiente pregunta necesaria. Usa emojis."""

        response = self.model.generate_content(prompt)
        return response.text.strip()
    
    def _extract_info(self, msg: str):
        """Extrae información del mensaje automáticamente"""
        ml = msg.lower()
        
        # Dispositivo
        if not self.data['device']:
            if 'laptop' in ml or 'portátil' in ml or 'portatil' in ml or 'notebook' in ml:
                self.data['device'] = 'Laptop'
            elif 'escritorio' in ml or 'torre' in ml or any(x in ml for x in ['pc ', ' pc', 'desktop', 'sobremesa']):
                self.data['device'] = 'PC Escritorio'
        
        # Sistema Operativo
        if not self.data['os']:
            if 'windows' in ml:
                if '11' in ml:
                    self.data['os'] = 'Windows 11'
                elif '10' in ml:
                    self.data['os'] = 'Windows 10'
                elif '7' in ml:
                    self.data['os'] = 'Windows 7'
                else:
                    self.data['os'] = 'Windows'
            elif 'linux' in ml or 'gnu' in ml:
                distros = {
                    'ubuntu': 'Ubuntu', 'debian': 'Debian', 'arch': 'Arch Linux',
                    'kali': 'Kali Linux', 'mint': 'Linux Mint', 'fedora': 'Fedora',
                    'manjaro': 'Manjaro', 'pop': 'Pop!_OS', 'zorin': 'Zorin OS'
                }
                ...
                if not self.data['os']:
                    self.data['os'] = 'Linux'
            elif 'mac' in ml or 'macos' in ml or 'osx' in ml:
                self.data['os'] = 'macOS'
        
        # RAM
        if not self.data['ram']:
            match = re.search(r'(\d+)\s*gb', ml)
            if match:
                self.data['ram'] = f"{match.group(1)} GB"
        
        # CPU
        if not self.data['cpu']:
            cpu_patterns = [
                (r'ryzen\s*([3579])\s*(\d{4})', 'AMD Ryzen'),
                (r'(i[3579])[- ]?(\d{4,5})', 'Intel Core'),
                (r'intel.*core.*([i][3579])', 'Intel Core'),
                (r'amd.*ryzen', 'AMD Ryzen')
            ]
            for pattern, prefix in cpu_patterns:
                match = re.search(pattern, ml, re.IGNORECASE)
                if match:
                    self.data['cpu'] = match.group(0).upper()
                    break
        
        # Fecha instalación
        if not self.data['install_date']:
            time_keywords = ['hace', 'años', 'meses', 'semana', 'días', 'ayer', 'recién', 'nuevo', 'viejo']
            if any(k in ml for k in time_keywords):
                # Extraer la frase relevante
                for keyword in time_keywords:
                    if keyword in ml:
                        idx = ml.find(keyword)
                        self.data['install_date'] = msg[max(0, idx-10):min(len(msg), idx+30)]
                        break
    
    def _is_complete(self) -> bool:
        """Verifica si hay suficiente información"""
        has_basic = bool(self.data['problem'] and self.data['device'] and self.data['os'])
        
        # Si es problema de rendimiento, necesitar RAM
        performance_keywords = ['lento', 'lag', 'rendimiento', 'traba', 'congela', 'freeze']
        if any(k in str(self.data['problem']).lower() for k in performance_keywords):
            return has_basic and bool(self.data['ram'])
        
        return has_basic
    
    def _fallback_question(self) -> str:
        """Preguntas manuales secuenciales"""
        if not self.data['device']:
            return "¿**Laptop** o **PC de escritorio**? 💻"
        elif not self.data['os']:
            return "¿Qué **sistema operativo** usas? 🖥️\n*(Windows 10/11, Linux, macOS)*"
        elif not self.data['ram'] and any(k in str(self.data['problem']).lower() for k in ['lento', 'lag', 'rendimiento']):
            return "¿Cuánta **RAM** tiene tu equipo? 💾\n*(Ej: 8GB, 16GB)*"
        elif not self.data['cpu']:
            return "¿Qué **procesador** tiene? ⚙️\n*(Ej: Intel i5, Ryzen 5)*"
        elif not self.data['install_date']:
            return "¿Hace cuánto instalaste el sistema operativo? 📅"
        else:
            return "COMPLETE"
    ...
    def get_summary(self) -> dict:
        ...
        return {
            'problem': self.data['problem'] or 'No especificado',
            'device': self.data['device'] or 'No especificado',
            'os': self.data['os'] or 'No especificado',
            'install_date': self.data['install_date'] or 'No especificado',
            'cpu': self.data['cpu'] or 'No especificado',
            'ram': self.data['ram'] or 'No especificado'
        }

# ============= SESIONES ACTIVAS =============

active_sessions = {}  # {channel_id: {'collector': InfoCollector, 'admin_id': int, 'user_id': int}}

# ============= COMANDOS =============

@tree.command(name="setupsupport", description="Configura el rol de soporte técnico")
@app_commands.describe(role="Rol que puede usar /startsupport")
@app_commands.checks.has_permissions(administrator=True)
async def setupsupport(interaction: discord.Interaction, role: discord.Role):
    """Configura rol de soporte"""
    if set_support_role(interaction.guild.id, role.id):
        embed = discord.Embed(
            title="✅ Rol de Soporte Configurado",
            description=f"El rol {role.mention} ahora puede usar `/startsupport`",
            color=discord.Color.green()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        logger.info(f"✅ Rol configurado en {interaction.guild.name}: {role.name}")
    else:
        await interaction.response.send_message("❌ Error al guardar configuración.", ephemeral=True)

@tree.command(name="startsupport", description="Inicia recopilación de información del usuario")
async def startsupport(interaction: discord.Interaction):
    """Inicia sesión de recopilación"""
    
    # Verificar permisos
    support_role_id = get_support_role(interaction.guild.id)
    ...
    if support_role_id:
        ...
        if support_role and support_role not in interaction.user.roles and not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(
                f"❌ Solo miembros con {support_role.mention} pueden usar este comando.",
                ephemeral=True
            )
            return
    elif not interaction.user.guild_permissions.moderate_members:
        await interaction.response.send_message(
            "❌ No tienes permisos. Usa `/setupsupport` primero.",
            ephemeral=True
        )
        return
    ...
    if interaction.channel_id in active_sessions:
        await interaction.response.send_message(
            "⚠️ Ya hay una sesión activa en este canal.",
            ephemeral=True
        )
        return
    ...
    active_sessions[interaction.channel_id] = {
        'collector': collector,
    ...
    embed = discord.Embed(
        title="🐢 Asistente de Información Técnica",
        description=(
            "¡Hola! Voy a hacerte algunas preguntas para entender mejor tu situación.\n\n"
    ...
    embed.set_footer(
        text=f"Iniciado por {interaction.user.name}",
        icon_url=interaction.user.display_avatar.url
    )
    
    await interaction.response.send_message(embed=embed)
    logger.info(f"✅ Sesión iniciada en {interaction.channel.name} por {interaction.user.name}")

@tree.command(name="endsupport", description="Finaliza la sesión de recopilación")
async def endsupport(interaction: discord.Interaction):
    """Termina sesión manualmente"""
    
    if interaction.channel_id not in active_sessions:
        await interaction.response.send_message(
            "❌ No hay sesión activa en este canal.",
            ephemeral=True
        )
        return
    
    session = active_sessions[interaction.channel_id]
    
    # Verificar permisos
    if interaction.user.id != session['admin_id'] and not interaction.user.guild_permissions.moderate_members:
        await interaction.response.send_message(
            "❌ Solo quien inició la sesión puede finalizarla.",
            ephemeral=True
        )
        return
    
    # Obtener info parcial
    collector = session['collector']
    summary = collector.get_summary()
    
    embed = discord.Embed(
        title="⚠️ Sesión Finalizada",
        description="Se recopiló la siguiente información:",
        color=0xFEE75C
    )
    
    if summary['problem'] != 'No especificado':
        embed.add_field(name="📝 Problema", value=summary['problem'], inline=False)
    if summary['device'] != 'No especificado':
        embed.add_field(name="💻 Dispositivo", value=summary['device'], inline=True)
    if summary['os'] != 'No especificado':
        embed.add_field(name="🖥️ Sistema", value=summary['os'], inline=True)
    
    await interaction.response.send_message(embed=embed)
    del active_sessions[interaction.channel_id]
    logger.info(f"⚠️ Sesión finalizada manualmente en {interaction.channel.name}")
...
async def on_message(message):
    ...
    # Asignar usuario en primer mensaje
    if not session.get('user_id'):
        session['user_id'] = message.author.id
    
    # Solo responder al usuario asignado
    if message.author.id != session['user_id']:
        return
    
    try:
        async with message.channel.typing():
            response = await collector.process(message.content)
        
        # Si completó la recopilación
        if response == "COMPLETE":
            summary = collector.get_summary()
            
            # Crear embed final bonito
            embed = discord.Embed(
                title="✨ Información Recopilada",
                description="Gracias por proporcionar la información. Un técnico revisará tu caso.",
                color=0x57F287,
                timestamp=datetime.now(timezone.utc)
            )
            
            # Campo principal: Problema
            embed.add_field(
                name="📋 Descripción del Problema",
                value=f"```{summary['problem']}```",
                inline=False
            )
            ...
            specs = []
            if summary['device'] != 'No especificado':
                specs.append(f"**Tipo:** {summary['device']}")
            if summary['os'] != 'No especificado':
                specs.append(f"**Sistema:** {summary['os']}")
            if summary['cpu'] != 'No especificado':
                specs.append(f"**Procesador:** {summary['cpu']}")
            if summary['ram'] != 'No especificado':
                specs.append(f"**Memoria RAM:** {summary['ram']}")
            
            if specs:
                embed.add_field(
                    name="💻 Especificaciones del Equipo",
                    value='\n'.join(specs),
                    inline=False
                )
            
            # Información adicional
            if summary['install_date'] != 'No especificado':
                embed.add_field(
                    name="📅 Instalación del Sistema",
                    value=summary['install_date'],
                    inline=False
                )
            
            # Footer con info del usuario
            embed.set_footer(
                text=f"Usuario: {message.author.name} • ID: {message.author.id}",
                icon_url=message.author.display_avatar.url
            )
            
            # Thumbnail decorativo
            embed.set_thumbnail(url="https://em-content.zobj.net/source/twitter/376/check-mark-button_2705.png")
            ...
            logger.info(f"✅ Info recopilada de {message.author.name} en {message.channel.name}")
        ...
        logger.error(f"❌ Error en recopilación: {e}")
        await message.channel.send(
            f"{message.author.mention} ❌ Ocurrió un error. Intenta `/startsupport` nuevamente."
        )
...
logger.info("✅ Sistema de recopilación de información cargado")
