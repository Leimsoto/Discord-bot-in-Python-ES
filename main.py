...
#------------------Esto es dependiente del anterior, es para permitir usar el comando /chat en ciertos canales-----------#
...
def get_support_role(guild_id: int) -> int:
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT support_role_id FROM guild_config WHERE guild_id = ?
            """, (guild_id,))
            result = cursor.fetchone()
...
class SupportModal(Modal, title="🐢 Formulario de Soporte Técnico"):
    def __init__(self, support_role_id: int, tech_user: discord.Member):
    ...
    problem = TextInput(
        ...
        placeholder="Describe brevemente el problema (ej: PC muy lento, error al iniciar)",
        required=True,
        max_length=500,
        min_length=10
    )
    ...
    device = TextInput(
        ...
        placeholder="Laptop, PC de escritorio, etc.",
        required=True,
        max_length=50
    )
    
    # Campo 3: Sistema Operativo
    os_info = TextInput(
        label="Sistema Operativo",
        style=discord.TextStyle.short,
        placeholder="Windows 10/11, Ubuntu 22.04, macOS, etc.",
        required=True,
        max_length=100
    )
    
    # Campo 4: Especificaciones
    specs = TextInput(
        label="CPU y RAM (usa 'neofetch' si no sabes)",
        style=discord.TextStyle.paragraph,
        placeholder="Ej: Intel i5-10400, 16GB RAM\nTip: Ejecuta 'neofetch' en terminal",
        required=False,
        max_length=200
    )
    
    # Campo 5: Información adicional
    additional = TextInput(
        label="¿Cuándo instalaste el SO? Info adicional",
        style=discord.TextStyle.paragraph,
        placeholder="Cualquier detalle relevante (opcional)",
        required=False,
        max_length=300
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Agregar avatar del usuario
            embed.set_thumbnail(url=interaction.user.display_avatar.url)
            
            # Campos del formulario
            embed.add_field(
                name="🔴 Problema Reportado",
                value=f"```{self.problem.value[:200]}```",
                inline=False
            )
            
            embed.add_field(
                name="💻 Dispositivo",
                value=f"`{self.device.value}`",
                inline=True
            )
            
            embed.add_field(
                name="🖥️ Sistema Operativo",
                value=f"`{self.os_info.value}`",
                inline=True
            )
            ...
            if self.additional.value:
                embed.add_field(
                    ...
                    value=f"```{self.additional.value[:200]}```",
                    inline=False
                )
            ...
            embed.set_footer(
                text=f"Ticket ID: {ticket_id} | Iniciado por: {self.tech_user.name}",
                icon_url=self.tech_user.display_avatar.url
            )
            
            # Botones de acción
            view = TicketActionsView(interaction.user.id, self.support_role_id)
            
            # Responder al usuario
            await interaction.response.send_message(
                f"✅ {interaction.user.mention} ¡Tu ticket ha sido creado exitosamente!",
                ephemeral=True
            )
            
            # Enviar el ticket al canal con mención al rol de soporte
            support_role = interaction.guild.get_role(self.support_role_id)
            message = await interaction.channel.send(
                content=f"🔔 {support_role.mention if support_role else '@Soporte'} - Nuevo ticket de soporte",
                embed=embed,
                view=view
            )
            ...
            await interaction.response.send_message(
                "❌ Error al crear el ticket. Intenta de nuevo.",
                ephemeral=True
            )
...
@tree.command(name="techsupport", description="Inicia un ticket de soporte técnico")
async def techsupport(interaction: discord.Interaction):
    """Comando principal de soporte técnico"""
    
    # Verificar rol configurado
    ...
    if support_role_id:
        ...
        if support_role not in interaction.user.roles and not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(
                f"❌ Solo miembros con el rol {support_role.mention} pueden usar este comando.",
                ephemeral=True
            )
            return
    elif not interaction.user.guild_permissions.moderate_members:
        await interaction.response.send_message(
            "❌ No tienes permisos. Un admin debe configurar el rol con `/setupsupport`",
            ephemeral=True
        )
        return
    ...
    embed = discord.Embed(
        title="🐢 Sistema de Soporte Técnico - Las Tortuguitas de Ezku",
        description=(
            "¡Bienvenido al sistema de soporte técnico!\n\n"
            "**📋 Información necesaria:**\n"
            "• Descripción clara del problema\n"
            "• Tipo de dispositivo (Laptop/PC)\n"
            "• Sistema operativo y versión\n"
            "• Especificaciones (CPU/RAM)\n\n"
            "💡 **Tip:** Si no conoces tus especificaciones, ejecuta:\n"
            "```bash\nneofetch\n```\n"
            "en tu terminal para obtener toda la información.\n\n"
            "**Haz clic en el botón para abrir el formulario** ⬇️"
        ),
        color=discord.Color.blue()
    )
    
    embed.set_thumbnail(url=interaction.guild.icon.url if interaction.guild.icon else None)
    embed.set_footer(text="El formulario expira en 5 minutos")
    
    # Crear vista con botón
    view = SupportStartView(support_role_id, interaction.user)
...
@app_commands.describe(role="Rol que puede usar /techsupport y atender tickets")
@app_commands.checks.has_permissions(administrator=True)
async def setupsupport(interaction: discord.Interaction, role: discord.Role):
    ...
    if set_support_role(interaction.guild.id, role.id):
        embed = discord.Embed(
            ...
            description=(
                f"**Rol de soporte:** {role.mention}\n\n"
                f"Los miembros con este rol ahora pueden:\n"
                f"• Usar `/techsupport` para iniciar tickets\n"
                f"• Tomar y resolver tickets de usuarios\n"
                f"• Recibir notificaciones de nuevos tickets"
            ),
            color=discord.Color.green()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        await interaction.response.send_message(
            "❌ Error al configurar el rol. Intenta de nuevo.",
            ephemeral=True
        )

# ============= ACTUALIZAR TABLA DE BD =============

def update_guild_config_table():
    """Añade campo support_role_id a guild_config si no existe"""
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            ...
            if 'support_role_id' not in columns:
                cursor.execute("""
                    ALTER TABLE guild_config 
                    ADD COLUMN support_role_id INTEGER
                """)
                logger.info("✅ Campo support_role_id agregado a guild_config")
    except Exception as e:
        logger.error(f"Error actualizando tabla: {e}")

# Llamar al iniciar
update_guild_config_table()
